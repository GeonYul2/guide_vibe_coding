...
- External GitHub repositories are never vendored, cloned, mirrored, or executed by the weekly radar.
- Radar output stores metadata only: names, URLs, descriptions, topics, stars, and review status.
...
- `.github/workflows/weekly-repo-radar.yml` needs `contents: write` and `pull-requests: write` only to create a review branch and PR under `repos/radar/`.
- The weekly radar uses the default `GITHUB_TOKEN`; do not add PATs, cloud keys, production secrets, or internal API credentials.
- The weekly radar calls only the GitHub Search API and writes candidate review artifacts. It does not execute code from candidate repositories.

## Company Adoption Checklist

Before enabling this in a company repository:

1. Review both GitHub Actions workflows.
2. Keep Actions disabled if your security team has not approved scheduled jobs.
...
5. Keep internal documents, secrets, customer data, and credentials out of `tasks/`, `sources/`, and `repos/radar/`.
6. If the company requires stricter control, disable `.github/workflows/weekly-repo-radar.yml` and run `scripts/weekly_repo_radar.py` manually in a sandbox.
